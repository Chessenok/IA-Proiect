...
fig.suptitle('Tipuri de grafice Matplotlib — Dataset Iris', fontsize=16, fontweight='bold')

# --- Subplot 1: Grafic linie — media lungimii sepalei per specie ---
medii = iris.groupby('species')['sepal_length'].mean()
axes[0, 0].plot(medii.index, medii.values, marker='o', linewidth=2, markersize=8, color='steelblue')
...
for specie, culoare in culori.items():
    # AICI era eroarea: liniile de mai jos trebuie să fie indentate cu un Tab sau 4 spații
...
axes[1, 1].hist(iris['sepal_length'], bins=20, color='steelblue', edgecolor='black', alpha=0.7)
...
axes[1, 1].axvline(medie_sepala, color='red', linestyle='--', linewidth=2, label=f'Medie: {medie_sepala:.2f}')
axes[1, 1].legend()
